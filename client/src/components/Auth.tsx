...
import { LogIn, ExternalLink, Shield, Info, Github } from 'lucide-react';
...
const Auth = () => {
  ...
  // SSO Configuration
  const SSO_BASE_URL = 'https://identity.akshatmehta.com';
  ...
  const handleSSOLogin = (provider: 'google' | 'github') => {
    setIsLoading(true);
    try {
      const callbackUrl = `${CLIENT_BASE_URL}/auth/success`;
      const finalCallbackUrl = redirectUrl 
        ? `${callbackUrl}?redirect=${encodeURIComponent(redirectUrl)}`
        : callbackUrl;
      
      // Construct the SSO URL directly
      const ssoUrl = `${SSO_BASE_URL}/api/oauth/${provider}?redirect=${encodeURIComponent(finalCallbackUrl)}`;
